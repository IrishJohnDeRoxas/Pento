defmodule PentoWeb.CoreComponents do
  ...
  def button(assigns) do
    ...
    >
      <div
        role="status"
        class="sr-only phx-submit-loading:not-sr-only flex items-center justify-center gap-1"
      >
        <svg
          aria-hidden="true"
        ...
        <span class="sr-only phx-submit-loading:not-sr-only">{@disable_with}</span>
      </div>
      <span class="not-sr-only phx-submit-loading:sr-only">{render_slot(@inner_block)}</span>
    </button>
    """
  end
